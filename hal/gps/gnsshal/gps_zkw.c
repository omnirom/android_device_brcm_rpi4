...
#define  NMEA_DEBUG 0
#if NMEA_DEBUG
#define NMEA_DBG(f, ...) ALOGD("%s: line = %d, " f, __func__, __LINE__, ##__VA_ARGS__)
#else
#define NMEA_DBG(...)    ((void)0)
#endif
                                  ...
                                  Token azimuth, Token snr)
{
        ...
        if ((prn > 0) && (prn < 32)) {
                ...
                NMEA_DBG("sv_status: ignore (%d)", prn);
                return 0;
        }
        ...
        r->sv_count++;
        NMEA_DBG("sv_status(%2d): %2d, %d, %2f, %3f, %2f, %d",
...
nmea_reader_parse(NmeaReader* const r)
{
        ...
        NMEA_DBG("Received: '%.*s'", r->pos, r->in);
        ...
        {
                int  n;
                NMEA_DBG("Found %d tokens", tzer->count);
                for (n = 0; n < tzer->count; n++) {
                        Token  tok = nmea_tokenizer_get(tzer, n);
                        NMEA_DBG("%2d: '%.*s'", n, tok.end-tok.p, tok.p);
                }
        }
#endif
        ...
        if (!memcmp(tok.p, "GGA", 3)) {
                ...
                if (LOC_FIXED(r)) {  /* 1: No fix; 2: 2D; 3: 3D*/
                        ...
                        for (idx = 0; idx < max; idx++) {
                                ...
                                if (tok_satellite.p == tok_satellite.end) {
                                        NMEA_DBG("GSA: found %d active satellites\n", idx);
                                        break;
                                }
                                ...
                                if (sate_id >= 1 && sate_id <= 32) {     // GP
                                        ...
                                        NMEA_DBG("GSA: invalid sentence, ignore!!");
                                        break;
                                }
                ...
                NMEA_DBG("in RMC, fixStatus=%c", tok_fixStatus.p[0]);
                ...
                if (seq == num) {
                        if (r->sv_count <= cnt) {
                                NMEA_DBG("r->sv_count = %d", r->sv_count);
        ...
        // Add for Accuracy
                ...
                if ((r->fix_mode == 3) || (r->fix_mode == 2)) {
                        ...
                        NMEA_DBG("GPS get accuracy from driver:%f\n", r->fix.accuracy);
                }
                else {
                        ERR("GPS get accuracy failed, fix mode:%d\n", r->fix_mode);
                }
        }
        else {
                tok.p -= 2;
                NMEA_DBG("unknown sentence '%.*s", tok.end-tok.p, tok.p);
        }
        ...
        if (r->location_can_report) {
                ...
                if (!(r->fix.flags & GPS_LOCATION_HAS_ACCURACY)) {
                    ...
                    NMEA_DBG("GPS add fake accuracy\n");
                }
        ...
        NMEA_DBG("r->sv_status_gnss.num_svs = %d, gps_nmea_end_tag = %d, sv_status_can_report = %d", r->sv_status_gnss.num_svs, gps_nmea_end_tag, r->sv_status_can_report);
        if (r->sv_status_can_report) {
                r->sv_status_can_report = 0;
                if (r->sv_status_gnss.num_svs != 0) {
                        r->sv_status_gnss.size = sizeof(GnssSvStatus);
                        NMEA_DBG("Report sv status");
...
nmea_reader_addc(NmeaReader* const r, int  c)
{
        ...
        if ((r->pos >= (int) sizeof(r->in)-1 ) || (r->pos < 0)) {
                r->overflow = 1;
                r->pos      = 0;
                NMEA_DBG("nmea sentence overflow\n");
                return;
        }
...
gps_state_start(GpsState*  s)
{
        ...
        started = 1;
        s->thread = callback_backup.create_thread_cb(gps_native_thread, gps_state_thread, s);
        if (!s->thread) {
...
gps_state_stop(GpsState*  s)
{
        ...
        if (ret != 1)
                ALOGE("could not send CMD_STOP command: %s",strerror(errno));
}
...
epoll_register(int  epoll_fd, int  fd)
{
        ...
        if (ret < 0)
                ALOGE("epoll ctl error: %s", strerror(errno));
        return ret;
}
...
gps_state_thread(void*  arg)
{
        ...
        if (epoll_register(epoll_fd, control_fd) < 0)
                ALOGE("epoll register control_fd error: %s", strerror(errno));
        if (epoll_register(epoll_fd, gps_fd) < 0)
                ALOGE("epoll register gps_fd error: %s", strerror(errno));
...
Exit:
        ...
        if (epoll_deregister(epoll_fd, control_fd) < 0)
                ALOGE("epoll deregister control_fd error: %s", strerror(errno));
        if (epoll_deregister(epoll_fd, gps_fd) < 0)
                ALOGE("epoll deregister gps_fd error: %s", strerror(errno));
        return;
}
...
gps_state_init(GpsState*  state)
{
    char path[PROPERTY_VALUE_MAX];
        ...
        ALOGI("Open gps hardware:  %s", path);
        ...
        if (state->fd < 0) {
                ALOGE("no gps hardware detected: %s:%d, %s", path, state->fd, strerror(errno));
                return;
        }
